...
public static class MapSharer
{
    ...
    private static readonly List<string> MapDownloadQueue = [];
    private static readonly List<Map> MapUploadQueue = [];
    private static readonly List<string> UploadedMaps = [];
    ...
    {
        using FileStream zipFileStream = new(zipName, FileMode.CreateNew, FileAccess.Write);
        using ZipArchive archive = new(zipFileStream, ZipArchiveMode.Create);
        _ = archive.CreateEntryFromFile(SafePath.CombineFilePath(ProgramConstants.GamePath, file), file);
    }
