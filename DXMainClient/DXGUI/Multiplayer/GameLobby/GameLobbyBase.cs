...
public abstract class GameLobbyBase : INItializableWindow
{
    ...
    protected virtual PlayerInfo FindLocalPlayer()
    {
        return Players.Find(p => p.Name == ProgramConstants.PLAYERNAME);
    }
    ...
    {
        return ddGameModeMapFilter.SelectedItem?.Text == FavoriteMapsLabel;
    }

    private List<GameModeMap> GetFavoriteGameModeMaps()
    {
        return GameModeMaps.Where(gmm => gmm.IsFavorite).ToList();
    }
    ...
    protected void HandleGameOptionPresetSaveCommand(GameOptionPresetEventArgs e)
    {
        HandleGameOptionPresetSaveCommand(e.PresetName);
    }
    ...
    protected void HandleGameOptionPresetLoadCommand(GameOptionPresetEventArgs e)
    {
        HandleGameOptionPresetLoadCommand(e.PresetName);
    }
    ...
    protected void ListMaps()
    {
        ...
        for (int i = 0; i < maps.Count; i++)
        {
            ...
            XNAListBoxItem rankItem = new()
            {
                ...
                ? StatisticsManager.Instance.HasBeatCoOpMap(gameModeMap.Map.UntranslatedName, gameModeMap.GameMode.UntranslatedUIName)
    ...
    private void DeleteMapConfirmation()
    {
        ...
        XNAMessageBox messageBox = XNAMessageBox.ShowYesNoDialog(WindowManager, "Delete Confirmation".L10N("Client:Main:DeleteMapConfirmTitle"),
    ...
    private void LbGameModeMapList_HoveredIndexChanged(object sender, EventArgs e)
    {
        ...
        GameModeMap gmm = (GameModeMap)lbGameModeMapList.GetItem(1, lbGameModeMapList.HoveredIndex).Tag;

        mapListTooltip.Text = gmm.Map.UntranslatedName != gmm.Map.Name
            ? "Original name:".L10N("Client:Main:OriginalMapName") + " " + gmm.Map.UntranslatedName
            : string.Empty;
    }
    ...
    private List<Map> GetMapList(int playerCount)
    {
        ...
        return mapList.Count < 1 && playerCount <= MAX_PLAYER_COUNT ? GetMapList(playerCount + 1) : mapList;
    }
    ...
    protected void InitPlayerOptionDropdowns()
    {
        ...
        for (int i = MAX_PLAYER_COUNT - 1; i > -1; i--)
        {
            ...
            {
                Name = "ddPlayerName" + i,
                ClientRectangle = new Rectangle(locationX,
        ...
        XNALabel lblName = GeneratePlayerOptionCaption("lblName", "PLAYER".L10N("Client:Main:PlayerOptionPlayer"), ddPlayerNames[0].X, playerOptionCaptionLocationY);
        XNALabel lblSide = GeneratePlayerOptionCaption("lblSide", "SIDE".L10N("Client:Main:PlayerOptionSide"), ddPlayerSides[0].X, playerOptionCaptionLocationY);
        XNALabel lblColor = GeneratePlayerOptionCaption("lblColor", "COLOR".L10N("Client:Main:PlayerOptionColor"), ddPlayerColors[0].X, playerOptionCaptionLocationY);

        XNALabel lblStart = GeneratePlayerOptionCaption("lblStart", "START".L10N("Client:Main:PlayerOptionStart"), ddPlayerStarts[0].X, playerOptionCaptionLocationY);
        lblStart.Visible = false;

        XNALabel lblTeam = GeneratePlayerOptionCaption("lblTeam", "TEAM".L10N("Client:Main:PlayerOptionTeam"), ddPlayerTeams[0].X, playerOptionCaptionLocationY);
    ...
    {
        return playerIndex < Players.Count
            ? Players[playerIndex]
            : playerIndex < Players.Count + AIPlayers.Count ? AIPlayers[playerIndex - Players.Count] : null;
    }

    protected PlayerExtraOptions GetPlayerExtraOptions()
    {
        return PlayerExtraOptionsPanel == null ? new PlayerExtraOptions() : PlayerExtraOptionsPanel.GetPlayerExtraOptions();
    }
    ...
    {
        return GetPlayerExtraOptions()?.GetTeamMappingsError();
    }

    private Texture2D LoadTextureOrNull(string name)
    {
        return AssetLoader.AssetExists(name) ? AssetLoader.LoadTexture(name) : null;
    }
    ...
    private int GetSpectatorSideIndex()
    {
        return SideCount + RandomSelectorCount;
    }
    ...
    {
        return pInfo.SideId == GetSpectatorSideIndex();
    }
    ...
    private void WriteMap(PlayerHouseInfo[] houseInfos)
    {
        ...
        IniFile globalCodeIni = new(SafePath.CombineFilePath(ProgramConstants.GamePath, "INI", "Map Code", "GlobalCode.ini"));
        ...
        if (isMultiplayer)
        {
            IniFile mpGlobalCodeIni = new(SafePath.CombineFilePath(ProgramConstants.GamePath, "INI", "Map Code", "MultiplayerGlobalCode.ini"));
    ...
    {
        FileInfo mapFileInfo = new(mapIni.FileName);
    ...
    private static IEnumerable<string> GetSupplementalMapFiles(string basePath, string baseFileName)
    {
        ...
        List<string> supplementalMapFileNames = ClientConfiguration.Instance.SupplementalMapFileExtensions
    ...
    private void ManipulateStartingLocations(IniFile mapIni, PlayerHouseInfo[] houseInfos)
    {
        ...
        IniFile spawnIni = new(SafePath.CombineFilePath(ProgramConstants.GamePath, ProgramConstants.SPAWNER_SETTINGS));
    ...
    protected GameType GetGameType()
    {
        ...
        return teamCount == 0 ? GameType.FFA : teamCount == 1 ? GameType.Coop : GameType.TeamGame;
    }
    ...
    protected int GetRank()
    {
        ...
        if (isMultiplayer)
        {
            ...
            if (Map.MaxPlayers <= 3)
            {
                ...
                return localTeamIndex > 0 && filteredPlayers.Count(p => p.TeamId == localTeamIndex) > 1 ? RANK_NONE : RANK_HARD;
            }
