...
public abstract class MultiplayerGameLobby : GameLobbyBase, ISwitchable
{
    ...
    public override void Initialize()
    {
        ...
        {
            XNAPlayerSlotIndicator indicatorPlayerReady = new(WindowManager)
            {
                Name = "playerStatusIndicator" + i,
                ClientRectangle = new Rectangle(statusIndicatorX, ddPlayerTeams[i].Y + statusIndicatorY,
                0, 0)
            };
    ...
    private void TbChatInput_EnterPressed(object sender, EventArgs e)
    {
        ...
        if (tbChatInput.Text.StartsWith("/"))
        {
            ...
            StringBuilder sb = new("To use a command, start your message with /<command>. Possible chat box commands:".L10N("Client:Main:ChatboxCommandTipText") + " ");
            foreach (ChatBoxCommand chatBoxCommand in chatBoxCommands)
            {
    ...
    {
        return gameModeMap.Map.MaxPlayers > 3
            ? StatisticsManager.Instance.GetCoopRankForDefaultMap(gameModeMap.Map.UntranslatedName, gameModeMap.Map.MaxPlayers)
            : StatisticsManager.Instance.HasWonMapInPvP(gameModeMap.Map.UntranslatedName, gameModeMap.GameMode.UntranslatedUIName, gameModeMap.Map.MaxPlayers)
            ? 2
            : -1;
    }
    ...
    {
        btnLaunchGame.Enabled = IsHost
            ? base.UpdateLaunchGameButtonStatus() && GameMode != null && Map != null
            : base.UpdateLaunchGameButtonStatus() && !chkAutoReady.Checked;
