...
public class PrivateMessagingWindow : XNAWindow, ISwitchable
{
    ...
    /// </summary>
    private readonly List<PrivateMessageUser> privateMessageUsers = [];
    ...
    {
        return !string.IsNullOrEmpty(playerName) && connectionManager.UserList.Find(u => u.Name == playerName) != null;
    }
    ...
    private Predicate<XNAListBoxItem> MatchItemForName(string userName)
    {
        return item => ((IRCUser)item.Tag)?.Name == userName;
    }

    private XNAListBoxItem FindItemForName(string userName)
    {
        return lbUserList.Items.Find(MatchItemForName(userName));
    }

    private int FindItemIndexForName(string userName)
    {
        return lbUserList.Items.FindIndex(MatchItemForName(userName));
    }
    ...
    private void TbMessageInput_EnterPressed(object sender, EventArgs e)
    {
        ...
        ChatMessage sentMessage = new(ProgramConstants.PLAYERNAME,
    ...
    {
        return user.GameID < 0 || user.GameID >= gameCollection.GameList.Count
            ? unknownGameIcon
            : gameCollection.GameList[user.GameID].Texture;
    }
    ...
    {
        return "Private Messaging".L10N("Client:Main:PrivateMessaging");
    }
