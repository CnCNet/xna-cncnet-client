...
internal class GameCreationWindow : XNAWindow
{
    ...
    private void BtnLoadMPGame_LeftClick(object sender, EventArgs e)
    {
        ...
        IniFile spawnSGIni =
            new(SafePath.CombineFilePath(ProgramConstants.GamePath, ProgramConstants.SAVED_GAME_SPAWN_INI));

        string password = Utilities.CalculateSHA1ForString(
            spawnSGIni.GetStringValue("Settings", "GameID", string.Empty))[..10];

        GameCreationEventArgs ea = new(gameName,
    ...
    private bool AllowLoadingGame()
    {
        ...
        return iniFile.GetStringValue("Settings", "Name", string.Empty) == ProgramConstants.PLAYERNAME
&& iniFile.GetBooleanValue("Settings", "Host", false);
    }
}