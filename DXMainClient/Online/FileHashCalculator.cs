...
public class FileHashCalculator
{
            ...
            foreach (DirectoryInfo translationFolder in translationsFolderPath.EnumerateDirectories())
            {
                foreach (TranslationGameFile tgf in translationGameFiles)
                {
                    ...
                    if (File.Exists(filePath))
                    {
                        ...
                        {
                            fileRelativePath = fileRelativePath[ProgramConstants.GamePath.Length..].TrimStart(Path.DirectorySeparatorChar);
                        }
    ...
    {
        return File.Exists(path) ? str + Utilities.CalculateSHA1ForFile(SafePath.CombineFilePath(ProgramConstants.GamePath, path)) : str;
    }
    ...
    {
        IniFile config = new(SafePath.CombineFilePath(ProgramConstants.GetBaseResourcePath(), CONFIGNAME));
