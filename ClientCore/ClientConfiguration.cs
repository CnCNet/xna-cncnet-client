...
public class ClientConfiguration
{
    ...
    {
        DirectoryInfo baseResourceDirectory = SafePath.GetDirectory(ProgramConstants.GetBaseResourcePath());
    ...
    /* 项目“ClientCore (net8.0-windows)”的未合并的更改
    在此之前:
    ...
    */
    ...
    在此之后:
            public (string Name, string Path) GetThemeInfoFromIndex(int themeIndex)
        {
            return clientDefinitionsIni.GetStringValue("Themes", themeIndex.ToString(), ",").Split(',').AsTuple2();
    */
    public (string Name, string Path) GetThemeInfoFromIndex(int themeIndex)
    {
        return clientDefinitionsIni.GetStringValue("Themes", themeIndex.ToString(), ",").Split(',').AsTuple2();
    ...
    public string GetThemePath(string themeName)
    {
        ...
        在此之后:
                    foreach (KeyValuePair<string, key in themeSection.Keys)
        */
        foreach (KeyValuePair<string, string> string > key {in }
            ...
            */
            ...
            在此之后:
                        if (osVersion.Major == 6 && osVersion.Minor <= 1)
                            return OSVersion.WIN7;
            */
            return osVersion.Major == 6 && osVersion.Minor <= 1 ? OSVersion.WIN7
 /* 项目“ClientCore (net8.0-windows)”的未合并的更改
 在此之前:
        ...
        */
        ...
        在此之后:
                if (p == 4 || p == 6 || p == 128)
                    return OSVersion.UNIX;
        */
        return p is 4 or 6 or 128 ? OSVersion.UNIX
 /* 项目“ClientCore (net8.0-windows)”的未合并的更改
 在此之前:
             return OSVersion.UNKNOWN;
 在此之后:
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            if (OperatingSystem.IsWindowsVersionAtLeast(6, 2))
            {
                ...
                在此之后:
                            ...
                            {
                                return (OperatingSystem.IsWindowsVersionAtLeast(5, 0) ? OSVersion.WINXP : OSVersion.UNKNOWN;
                */
                return OSVersion.WIN810;
            }
            else
            {
                return OperatingSystem.IsWindowsVersionAtLeast(6, 1)
                    ...
                    : OperatingSystem.IsWindowsVersionAtLeast(6, 0)
                                    ? OSVersion.WINVISTA
                                    : OperatingSystem.IsWindowsVersionAtLeast(5, 0) ? OSVersion.WINXP : OSVersion.UNKNOWN;
            }
        }
    }
