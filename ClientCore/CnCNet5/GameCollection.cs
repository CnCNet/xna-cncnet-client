...
public class GameCollection
{
    ...
    public void Initialize()
    {
        ...
        using Image tiIcon = Image.Load(tiIconStream);
        using Image tsIcon = Image.Load(tsIconStream);
        using Image moIcon = Image.Load(moIconStream);
        using Image yrIcon = Image.Load(yrIconStream);
        using Image rrIcon = Image.Load(rrIconStream);
        using Image reIcon = Image.Load(reIconStream);
        using Image cncrIcon = Image.Load(cncrIconStream);
        using Image cncnetIcon = Image.Load(cncnetIconStream);
        using Image tdIcon = Image.Load(tdIconStream);
        using Image raIcon = Image.Load(raIconStream);
        using Image d2kIcon = Image.Load(d2kIconStream);
        using Image ssIcon = Image.Load(ssIconStream);
        using Image unknownIcon = Image.Load(unknownIconStream);
    ...
    private string GetIRCChannelNameFromIniFile(IniFile iniFile, string section, string key)
    {
        ...
        return string.IsNullOrEmpty(channel)
            ...
            : channel.Contains(' ') || channel.Contains(',') || channel.Contains((char)7)
    ...
    public string GetGameNameFromInternalName(string gameName)
    {
        ...
        return game == null ? gameName : game.UIName;
    }
