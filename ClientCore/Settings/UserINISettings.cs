...
public class UserINISettings
{
    ...
    public static UserINISettings Instance => _instance ?? throw new InvalidOperationException("UserINISettings not initialized!");
    ...
    public static void Initialize(string iniFileName)
    {
        ...
        IniFile iniFile = new(SafePath.CombineFilePath(ProgramConstants.GamePath, iniFileName));
    ...
    protected UserINISettings(IniFile iniFile)
    {
        ...
        FavoriteMaps = new StringListSetting(iniFile, OPTIONS, "FavoriteMaps", []);
    }
    ...
    {
        return SettingsIni.GetStringValue(section, key, defaultValue);
    }

    public bool GetValue(string section, string key, bool defaultValue)
    {
        return SettingsIni.GetBooleanValue(section, key, defaultValue);
    }

    public int GetValue(string section, string key, int defaultValue)
    {
        return SettingsIni.GetIntValue(section, key, defaultValue);
    }
    ...
    public bool IsFavoriteMap(string nameName, string gameModeName)
    {
        return FavoriteMaps.Value.Contains(FavoriteMapKey(nameName, gameModeName));
    }

    private string FavoriteMapKey(string nameName, string gameModeName)
    {
        return $"{nameName}:{gameModeName}";
    }
