    ...
    public static void StartGameProcess(WindowManager windowManager)
    {
        ...
        if (UserINISettings.Instance.WindowedMode && UseQres && RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            QResProcess.StartInfo.Arguments = !string.IsNullOrEmpty(extraCommandLine)
                ? "c=16 /R " + "\"" + SafePath.CombineFilePath(ProgramConstants.GamePath, gameExecutableName) + "\" " + additionalExecutableName + "-SPAWN " + extraCommandLine
                : "c=16 /R " + "\"" + SafePath.CombineFilePath(ProgramConstants.GamePath, gameExecutableName) + "\" " + additionalExecutableName + "-SPAWN";
