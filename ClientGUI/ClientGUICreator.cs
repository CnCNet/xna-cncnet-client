...
{
    private static readonly List<Type> controlTypes = [];
    ...
    public static XNAControl GetXnaControl(string controlTypeName)
    {
        return GetXnaControl(serviceProvider, controlTypeName);
    }
    ...
    {
        return serviceProvider.GetService(type) ?? throw new Exception($"Control type {type.Name} was not registered with ServiceCollection in GameClass");
    }
}