...
> [!IMPORTANT]
> By default, the translation system will attempt to create a [hard link](https://learn.microsoft.com/en-us/windows/win32/fileio/hard-links-and-junctions) to the translation game files, and if linking fails — copy them. While this link is being created, the source and target files **can be edited**. Be aware that if you edit the source file or target link, an editor can either break the hard link or update source file. To see links in your Windows Explorer, you can install [extension](https://schinagl.priv.at/nt/hardlinkshellext/linkshellextension.html).
