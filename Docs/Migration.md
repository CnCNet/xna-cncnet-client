...
- Translation keys in `Translation.ini` `[Values]` now append a checksum suffix. Existing translation entries that still use the old unhashed keys will become outdated and stop matching after upgrading to 2.14.0. Regenerate the translation stub or rename existing translation keys to the new suffixed format when migrating to this version.
