...
internal class CnCNetOptionsPanel : XNAOptionsPanel
{
    ...
    private readonly List<XNAClientCheckBox> followedGameChks = [];
    ...
    private void InitGameListPanel()
    {
        ...
        List<List<XNAPanel>> gamePanelMatrix = supportedGames
            .Select(game =>
            {
                XNAPanel gameIconPanel = new(WindowManager)
                {
                    Name = "gameIcon" + game.InternalName.ToUpperInvariant(),
                    ClientRectangle = new Rectangle(0, 0, gameIconWidth, gameIconWidth),
                    DrawBorders = false,
                    BackgroundTexture = game.Texture
                };

                XNAClientCheckBox gameChkBox = new(WindowManager)
                {
                    Name = game.InternalName.ToUpperInvariant(),
                    ClientRectangle = new Rectangle(gameIconPanel.Right + gameIconBuffer, 0, 0, 0),
                    Text = game.UIName
                };

                XNAPanel gamePanel = new(WindowManager);
